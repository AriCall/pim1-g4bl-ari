...
#Tunes the last four magnets.  This one works!
...
def beam_deviance(scales):
    ...
    os.environ['SECONDTURNSCALE'] = str(scales[4])
    subprocess.run("g4bl $PIM1/piM1_mu.g4bl", shell=True, check=True)
    with uproot.open("piM1_plastic_decay_bend_air_mu+_100_1_300_12350.root") as file:
        ...
        y_std = np.std(y_vals)
        count = len(x_vals)
    os.remove("piM1_plastic_decay_bend_air_mu+_100_1_300_12350.root")
    return np.sqrt(x_std**2+y_std**2)-count/2

#Arbitrary bounds; if the minimum is found to be very close to 4 or 0.001 for any of them, I will change them.
bounds = [(0.1,3),(0.1,3),(0.1,3),(0.1,3),(0.1,3)]
